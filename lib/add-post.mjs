...
// History of hand-added posts (source_handle="manual"), newest-added first.
export async function listManualPosts(env = process.env) {
  const supabase = getAdminClient(env);
  const { data, error } = await supabase.from("posts")
    .select("id,platform,author,author_name,text,image,images,url,created_at,posted_at,likes,players,celebrities,keywords,category,hidden")
    .eq("source_handle", "manual")
...
// Hard-delete a manually added post (admin-curated, so a real remove).
export async function removeManualPost(id, env = process.env) {
  const supabase = getAdminClient(env);
  const { error } = await supabase.from("posts").delete().eq("id", id).eq("source_handle", "manual");
  if (error) throw error;
}
