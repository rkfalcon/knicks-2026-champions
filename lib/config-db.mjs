...
const DEFAULTS = {
  // "Latest" returns every tweet in the window chronologically; "Top" only the
  // algorithmically-popular ones (which silently drops posts).
  twitter: { queryType: "Latest", maxPagesPerQuery: 5 },
  instagram: { resultsLimitPerSource: 20 },
};
...
export async function loadConfigFromDb(sb) {
  ...
  // Normalize handles: strip a leading @ and surrounding whitespace so a value
  // like "@23savage____" still builds a valid from:/profile query.
  const cleanHandle = (h) => (h ? h.trim().replace(/^@+/, "") || null : null);
  const acc = (accounts.data || []).map((a) => ({
    ...a, x_handle: cleanHandle(a.x_handle), ig_handle: cleanHandle(a.ig_handle),
  }));
  ...
  return {
    ...
    // Every tracked handle (across ALL accounts, not just this run's subset) — used
    // to decide whether a collab post's owner is itself a tracked account.
    allTrackedHandles: new Set(acc.flatMap((a) =>
    ...
    showAll: new Set(acc.filter((a) => a.show_all).flatMap((a) =>
      [a.x_handle, a.ig_handle].filter(Boolean).map((h) => h.toLowerCase()))),
    // Handles flagged posts-only: keep their keyword-matching posts but skip
    // their (unfilterable) stories/highlights. For noisy brand/lifestyle accounts.
    postsOnly: new Set(acc.filter((a) => a.posts_only).flatMap((a) =>
      [a.x_handle, a.ig_handle].filter(Boolean).map((h) => h.toLowerCase()))),
    twitter: {
      ...DEFAULTS.twitter,
    ...
    instagram: {
      ...
      hashtags: [],
      postsActor: (S.stories && S.stories.posts_actor) || "muhammetakkurtt/instagram-scraper",
    },
