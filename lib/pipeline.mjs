...
import { mirrorPending } from "./mirror-pending.mjs";
import {
...
function tagAndDedupe(raw, config) {
  ...
  // Apply the date window here (we no longer ask the IG actor to filter by date
  // — see instagram.mjs). `since` is the rolling window for the cron, the full
  // window for backfills. Compare on the date portion so same-day posts pass.
  const since = config.dateRange?.since || null;
  const posts = raw
    .filter((p) => p.date && (!since || String(p.date).slice(0, 10) >= since)
      && (p.postType === "story" || p.postType === "highlight" || isRelevant(p, config)))
    .map((p) => {
...
export async function scrapeAndTag(config, env = process.env, { skipInstagram = false, timeLeft = () => Infinity } = {}) {
  // X and Instagram are independent — fetch them concurrently so the slower one
  // (IG) overlaps the X scrape instead of running strictly after it. Both stop
  // accepting new work as the shared time budget runs low.
  const [tweets, igPosts] = await Promise.all([
    fetchTwitter(config, env.TWITTERAPI_IO_KEY, { timeLeft }),
    skipInstagram ? Promise.resolve([]) : fetchInstagram(config, env.APIFY_TOKEN, { timeLeft }),
  ]);
...
export async function runIngest({
  ...
  // The full-history window, captured before the incremental path trims
  // config.dateRange.since below — Phase 0 backfill must use the FULL window,
  // not the rolling one, or new accounts only get the last few days of posts.
  const fullSince = config.dateRange.since;

  // Restrict to active accounts — and to one account if accountId is given.
  // For cron / run-all (no accountId), skip accounts flagged cron_enabled=false
  // (manual-only accounts that hold hand-added posts). An explicit per-account
  // Run still scrapes them, so you can pull one on demand without enabling cron.
  let accounts = (config.accounts || []).filter((a) => a.active);
  if (accountId) accounts = accounts.filter((a) => a.id === accountId);
  else accounts = accounts.filter((a) => a.cron_enabled !== false);
  // Least-recently-scraped first, so if a run runs out of budget and stops early,
  // the accounts it skips are the ones scraped most recently — coverage rotates
  // fairly across runs instead of always favoring the same head of the list.
  accounts.sort((a, b) => (a.last_scraped_at || "").localeCompare(b.last_scraped_at || ""));
  ...
  if (!full) {
    ...
    // Incremental batches only pull the latest 25 posts/profile, so they return
    // fast — a tight per-batch timeout bounds how far a slow batch can overrun.
    config.instagram = { ...config.instagram, resultsLimitPerSource: 25, batchTimeoutMs: 100000 };
  ...
  try {
    ...
    let backfillCount = 0;
    // Backfill is the heaviest discretionary work (full-history, deep fetch) — only
    // run it when there's ample budget left, so it can never starve the daily scrape.
    if (backfillNew && timeLeft() > 120000) {
      ...
      if (pending.length) {
        ...
        const bf = {
          ...config,
          accounts: pending,
          dateRange: { ...config.dateRange, since: fullSince }, // FULL history, not the rolling window
        ...
        const bfPosts = await scrapeAndTag(bf, env, { timeLeft });
    ...
    const posts = await scrapeAndTag(config, env, { skipInstagram, timeLeft });
    ...
    if (!skipInstagram && timeLeft() > 30000) {
      try {
        // With no deadline (manual run) timeLeft() is Infinity — setTimeout would
        // clamp that to ~1ms and skip stories instantly, so just run to completion.
        const stories = Number.isFinite(softDeadline)
          ? await Promise.race([
              scrapeStories(config, env),
              new Promise((resolve) => setTimeout(() => resolve(null), timeLeft() - 20000)),
            ])
          : await scrapeStories(config, env);
    ...
    // Phase 3 — mirror catch-up. Self-heals posts left blank by an earlier run's
    // image cap, and re-fetches expired source URLs. Uses leftover time budget so
    // the backlog clears over a night or two without manual sweeps.
    if (uploadMedia && timeLeft() > 30000) {
      try {
        const r = await mirrorPending(supabase, env, { limit: 150, refetchLimit: 15, timeLeft, log });
        mirroredTotal += r.frames;
      } catch (e) { log(`⚠ mirror catch-up failed: ${e.message}`); }
    }
