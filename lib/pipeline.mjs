...
export async function runIngest({
  ...
  // Mirror a batch's images to Storage, bounded by the run's shared image budget.
  let mirroredTotal = 0;
  const mirrorBatch = async (items) => {
    ...
    for (const p of items) {
      ...
      if (p.image && /^https?:/.test(p.image)) {
        ...
        if (url) { p.remoteImage = p.image; p.image = url; mirroredTotal++; changed.push(p); }
      }
    }
  ...
  try {
    ...
    if (posts.length) { await upsertPosts(supabase, posts); await mirrorBatch(posts); }

    // Phase 2 — Instagram stories/highlights (fault-isolated: its own try/catch).
    let storyCount = 0;
    try {
      const stories = await scrapeStories(config, env);
      if (stories.length) { await upsertPosts(supabase, stories); await mirrorBatch(stories); storyCount = stories.length; }
    } catch (e) {
