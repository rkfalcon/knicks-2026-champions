...
async function runBatch(actor, token, usernames, cookie, includeHighlights, attempt = 1) {
  const res = await fetchWithTimeout(runUrl(actor, token), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ usernames, sessionCookie: cookie, includeHighlights, maxHighlights: 5 }),
  }, 280000); // stories actor is slow; keep below the 300s serverless limit. In the
              // cron, the Phase-2 Promise.race time budget bounds the phase anyway.
  if (!res.ok) {
...
export async function fetchInstagramStories(config, env) {
  ...
  // Skip accounts flagged posts-only — their stories are unfilterable noise
  // (e.g. brand/merch accounts that post non-Knicks content).
  const postsOnly = config.postsOnly || new Set();
  const usernames = (config.instagram.profileHandles || []).filter((u) => !postsOnly.has((u || "").toLowerCase()));
